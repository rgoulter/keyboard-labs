...
pub fn erased_input_5<
    const AP: char, const AN: u8, AM,
    const BP: char, const BN: u8, BM,
    const CP: char, const CN: u8, CM,
    const DP: char, const DN: u8, DM,
    const EP: char, const EN: u8, EM,
>(
    a: Pin<AP, AN, AM>,
    b: Pin<BP, BN, BM>,
    c: Pin<CP, CN, CM>,
    d: Pin<DP, DN, DM>,
    e: Pin<EP, EN, EM>
) -> (EPin<Input>, EPin<Input>, EPin<Input>, EPin<Input>, EPin<Input>)
    where
    AM: PinMode,
    BM: PinMode,
    CM: PinMode,
...
pub fn row5_is_low(
    (a, b, c, d, e): &(EPin<Input>, EPin<Input>, EPin<Input>, EPin<Input>, EPin<Input>)
) -> [bool; 5] {
    [
        a.is_low(),
        b.is_low(),
...
pub fn row3_is_low_lhs(
    (a, b, c): &(EPin<Input>, EPin<Input>, EPin<Input>)
) -> [bool; 5] {
    [
        false,
        false,
